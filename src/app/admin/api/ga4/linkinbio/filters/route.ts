  ...
  try {
    ...
    const response = await runGa4ReportForProperty(propertyId, {
      ...
      dimensionFilter: combineDimensionFilters(
        { filter: { fieldName: "eventName", inListFilter: { values: ["store_filter_select"] } } },
        brandFilter,
      ),
    ...
    console.error("Linkinbio filters GA4 error:", error);
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "Failed to fetch filter data" },
      { status: 500 }
    );
  }
}
