...
export async function GET(request: Request) {
  ...
  try {
    ...
    const response = await runGa4ReportForProperty(propertyId, {
      ...
      dimensions: [{ name: "customEvent:store_name" }, { name: "customEvent:is_direct" }],
      metrics: [{ name: "eventCount" }],
      dimensionFilter: combineDimensionFilters(
        { filter: { fieldName: "eventName", inListFilter: { values: ["store_select"] } } },
        brandFilter,
      ),
    ...
    const data =
      response.rows?.map((row) => ({
        ...
        isDirect: (row.dimensionValues?.[1]?.value || "false") === "true",
        count: parseInt(row.metricValues?.[0]?.value || "0", 10),
      })) || [];

    return NextResponse.json({ success: true, data });
  } catch (error) {
    console.error("Linkinbio stores GA4 error:", error);
    ...
    return NextResponse.json(
      { success: false, error: error instanceof Error ? error.message : "Failed to fetch store data" },
      { status: 500 }
    );
  }
}
